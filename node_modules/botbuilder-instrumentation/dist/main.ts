...
export class BotFrameworkInstrumentation {
  ...
  private appInsightsClients: { [index: string]: typeof ApplicationInsights.client } = {};
  ...
  constructor(settings?: IInstrumentationSettings) {
    ...
    if (settings.instrumentationKey) {
      this.instrumentationKeys = Array.isArray(settings.instrumentationKey) ?
        settings.instrumentationKey : settings.instrumentationKey.split(',');
    } else if (process.env.APP_INSIGHTS_INSTRUMENTATION_KEYS) {
      this.instrumentationKeys = process.env.APP_INSIGHTS_INSTRUMENTATION_KEYS.split(',');
    }
  ...
  private setupInstrumentation() {
    if (this.instrumentationKeys && this.instrumentationKeys.length > 0) {
      ...
      console.log("INSTRUMENTATION KEYs: ", this.instrumentationKeys[0])
      ...
      _.forEach(this.instrumentationKeys, (iKey) => {
        this.appInsightsClients[iKey] = ApplicationInsights.getClient(iKey);
      });
    }
  }
  ...
  private trackEvent(
    ...
    _.forEach(Object.values(this.appInsightsClients), (client, index) => {
      console.log("CLIENT", this.instrumentationKeys[index], "EVENT :", name, "PROPS:", JSON.stringify(properties, null, 2))
      client.trackEvent(name, properties, measurements, tagOverrides, contextObjects);
    });
  }
  ...
  private trackTrace(
    ...
    contextObjects?: { [name: string]: any; }): void {
    _.forEach(Object.values(this.appInsightsClients), (client, index) => {
      console.log("CLIENT", this.instrumentationKeys[index], "EVENT :", message, "SEC-LEVEL: ", severityLevel, "PROPS :", JSON.stringify(properties, null, 2))
      client.trackTrace(message, severityLevel, properties, tagOverrides, contextObjects);
    });
  }
  ...
  private trackException(
    ...
    contextObjects?: { [name: string]: any; }): void {
    _.forEach(Object.values(this.appInsightsClients), (client, index) => {
      console.log("CLIENT", this.instrumentationKeys[index], "EVENT:", exception, "EXCEPTION: ", exception, "PROPS :", JSON.stringify(properties, null, 2))
      client.trackException(exception, properties, measurements, tagOverrides, contextObjects);
    });
  }
}